...
///var topics = require('skynode-contents/topics');
var notifications = require('../notifications');
///var messaging = require('skynode-collab/messaging');
...
var SocketUser = ns.user = module.exports;
...
SocketUser.exists = function (socket, data, callback) {
	...
	slugs.userOrGroupExists(data.username, callback);
};
...
SocketUser.reset.commit = function (socket, data, callback) {
	...
	async.waterfall([
		...
		function (username, next) {
			...
			user.email.send('reset_notify', uid, {
				username: username,
				date: parsedDate,
...
SocketUser.getUserByUID = function (socket, uid, callback) {
	privileges.users.getUserDataByField(socket.uid, 'uid', uid, callback);
};

SocketUser.getUserByUsername = function (socket, username, callback) {
	privileges.users.getUserDataByField(socket.uid, 'username', username, callback);
};

SocketUser.getUserByEmail = function (socket, email, callback) {
	privileges.users.getUserDataByField(socket.uid, 'email', email, callback);
};
