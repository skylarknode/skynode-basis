...
var nfs = require('skynode-basis/helpers/files/nfs');
var async = require('async');
var sanitizeHTML = require('skynode-basis/helpers/html/sanitize');
var nconf = require('skynode-basis/system/parameters');

var file = require('skynode-basis/file');
var Translator = require('skynode-basis/translator').Translator;
...
function getAdminNamespaces(callback) {
	file.walk(nfs.resolve(nconf.get('views_dir'), 'admin'), function (err, directories) {
		if (err) {
			return callback(err);
		}
...
function initFallback(namespace, callback) {
	nfs.readFile(nfs.resolve(nconf.get('views_dir'), namespace + '.tpl'), 'utf8', function (err, template) {
		if (err) {
			return callback(err);
		}
