...
Dependencies.check = function (callback) {
	console.log("pkg:" + path.join(nconf.get('base_dir'),"package.json"));
	var pkg = require(path.join(nconf.get('base_dir'),"package.json"));
...
Dependencies.checkModule = function (moduleName, callback) {
	...
	fs.readFile(path.join(nconf.get('base_dir'), 'node_modules/', moduleName, 'package.json'), {
		encoding: 'utf-8',
