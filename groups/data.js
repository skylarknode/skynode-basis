...
function modifyGroup(group, fields) {
	if (group) {
		...
		if (group['cover:url']) {
			...
			group['cover:url'] = require('skynode-basis/coverPhoto').getDefaultGroupCover(group.name);
		}
		...
		if (group['cover:thumb:url']) {
			...
			group['cover:thumb:url'] = require('skynode-basis/coverPhoto').getDefaultGroupCover(group.name);
		}
