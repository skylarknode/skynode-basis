...
UserEmail.sendValidationEmail = function (uid, options, callback) {
	...
	async.waterfall([
		...
		function (username, next) {
			var title = meta.config.title || meta.config.browserTitle || 'SkyBB';
			...
			if (plugins.hasListeners('action:user.verify')) {
				...
				UserEmail.send(template, uid, data, next);
			}
		},
		function (next) {
...
//moved from emailer.js
UserEmail.send = function (template, uid, params, callback) {
	callback = callback || function () {};
	if (!app) {
